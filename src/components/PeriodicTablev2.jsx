...
const elements = [
    ...
    // {num:58, id:'Ce', name:'Cerium', mass:'140.12', x:4 , y:5, isotopes: [R,G,B,R,G,B]},
    // {num:59, id:'Pr', name:'Praseodymium', mass:'140.91', x:5 , y:5, isotopes: [R,G,B,R,G,B]},
    // {num:60, id:'Nd', name:'Neodymium', mass:'144.24', x:6 , y:5, isotopes: [R,G,B,R,G,B]},
    // {num:61, id:'Pm', name:'Promethium', mass:'(145)', x:7 , y:5, isotopes: [R,G,B,R,G,B]},
    // {num:62, id:'Sm', name:'Samarium', mass:'150.36', x:8 , y:5, isotopes: [R,G,B,R,G,B]},
    // {num:63, id:'Eu', name:'Europium', mass:'151.96', x:9 , y:5, isotopes: [R,G,B,R,G,B]},
    // {num:64, id:'Gd', name:'Gadolinium', mass:'157.25', x:10 , y:5, isotopes: [R,G,B,R,G,B]},
    // {num:65, id:'Tb', name:'Terbium', mass:'158.93', x:11 , y:5, isotopes: [R,G,B,R,G,B]},
    // {num:66, id:'Dy', name:'Dysprosium', mass:'162.5', x:12 , y:5, isotopes: [R,G,B,R,G,B]},
    // {num:67, id:'Ho', name:'Holmium', mass:'164.93', x:13 , y:5, isotopes: [R,G,B,R,G,B]},
    // {num:68, id:'Er', name:'Erbium', mass:'167.26', x:14 , y:5, isotopes: [R,G,B,R,G,B]},
    // {num:69, id:'Tm', name:'Thulium', mass:'168.93', x:15 , y:5, isotopes: [R,G,B,R,G,B]},
    // {num:70, id:'Yb', name:'Ytterbium', mass:'173.05', x:16 , y:5, isotopes: [R,G,B,R,G,B]},
    // {num:71, id:'Lu', name:'Lutetium', mass:'174.97', x:17 , y:5, isotopes: [R,G,B,R,G,B]},
    ...
    // {num:90, id:'Th', name:'Thorium', mass:'232.04', x:4 , y:4, isotopes: [R,G,B,R,G,B]},
    // {num:91, id:'Pa', name:'Protactinium', mass:'231.06', x:5 , y:4, isotopes: [R,G,B,R,G,B]},
    // {num:92, id:'U', name:'Uranium', mass:'238.028', x:6 , y:4, isotopes: [R,G,B,R,G,B]},
    // {num:93, id:'Np', name:'Neptunium', mass:'(237)', x:7 , y:4,  isotopes: [R,G,B,R,G,B]},
    // {num:94, id:'Pu', name:'Plutonium', mass:'(244)', x:8 , y:4, isotopes: [R,G,B,R,G,B]},
    // {num:95, id:'Am', name:'Americium', mass:'(243)', x:9 , y:4, isotopes: [R,G,B,R,G,B]},
    // {num:96, id:'Cm', name:'Curium', mass:'(247)', x:10 , y:4, isotopes: [R,G,B,R,G,B]},
    // {num:97, id:'Bk', name:'Berkelium', mass:'(247)', x:11 , y:4, isotopes: [R,G,B,R,G,B]},
    // {num:98, id:'Cf', name:'Californium', mass:'(251)', x:12 , y:4, isotopes: [R,G,B,R,G,B]},
    // {num:99, id:'Es', name:'Einstenium', mass:'(252)', x:13 , y:4, isotopes: [R,G,B,R,G,B]},
    // {num:100, id:'Fm', name:'Fermium', mass:'(257)', x:14 , y:4, isotopes: [R,G,B,R,G,B]},
    // {num:101, id:'Md', name:'Mendelevium', mass:'(258)', x:15 , y:4, isotopes: [R,G,B,R,G,B]},
    // {num:102, id:'No', name:'Nobelium', mass:'(259)', x:16 , y:4, isotopes: [R,G,B,R,G,B]},
    // {num:103, id:'Lr', name:'Lawrencium', mass:'(262)', x:17 , y:4, isotopes: [R,G,B,R,G,B]},
...
const data = Array.from({ length: 500 }, () => ({ color:'#f0f0f0', scale: 1}))
const size = 500
...
function Boxes() {
    ...
    const colorArray = useMemo(() => Float32Array.from(new Array(500).fill().flatMap((_, i) => tempColor.set(data[i].color).toArray())), [])
    ...
    for(let c = 0;c < elements.length;c++) {
        const id = i++
            ...
            // }
            const scale = (data[id].scale = THREE.MathUtils.lerp(data[id].scale, id === hovered ? 1.5 : 1, 0.25))
